(function (global, factory) {
  ...
  $RefParser = $RefParser && Object.prototype.hasOwnProperty.call($RefParser, 'default') ? $RefParser['default'] : $RefParser;
  ...
  function objectType(value, path, resolve, traverseCallback) {
    ...
    var _limit = optionalsProbability !== false ? max : random.number(0, max);
    var _props = requiredProperties.concat(extraProperties.slice(0, _limit));
    var _defns = [];
      ...
      var result = traverse(utils.clone(schema), [], function reduce(sub, index, rootPath) {
        ...
        if (Array.isArray(sub.allOf)) {
          ...
          schemas.forEach(function (subSchema) {
            var _sub = reduce(subSchema, null, rootPath);
